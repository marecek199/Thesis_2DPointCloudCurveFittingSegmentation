...
def isLocalMax(Acum,index1,index2,velkost,data_f):
    ...
    # Image.fromarray(dataaa,'RGB').show()
    concl = True
    pocitadlo = 0
    for i in range(i0,i1):
        for j in range(j0,j1):
    ...
    # dataaa[index1-i0][index2-j0][0] = 255
    # dataaa[index1-i0][index2-j0][1] = 0
    # dataaa[index1-i0][index2-j0][2] = 0
    if pocitadlo > 10:
        return concl
    return False
...
def isLocalMax2(Acum,index1,index2,velkost,data_f):
    ...
    # dataaa[index1-i0][index2-j0][2] = 0
    # if concl:
    #vykreslenie konecneho vybrateho bodu ???
    #     Image.fromarray(dataaa,'RGB').show()
    if concl:
...
def vykresli_accum(accum,maxim,data_f_a):
    ...
    for riadok in range(riad):
        for stlpec in range(stlp):
            ...
            # imaged[riadok][stlpec] = int(accum[riadok][stlpec]* 255 // maxim)
    # Image.fromarray(imaged,'L').show()
...
#vybratie množiny meraní [x:y]    , maximalna hodnota y=106
