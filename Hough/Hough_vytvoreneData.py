...
def kolinear(pole):
    ...
    treshold_vzdialenost_priamok = 5*min_vzdial*2
    tolerancna_medz_so_smernicou = 3*min_vzdial*2
    tolerancna_medz_bez = -2*min_vzdial*2
    treshold_body = 7*min_vzdial*2
...
def vykresli_usecky(pole_useciek,x_r,y_r):
    global w,h,zobaz_AkumulMaticu
    pole_delenie_udaje = []
    data_usecka = np.zeros((w,h), dtype=np.uint8)   
    for idx in pole_useciek:
        fi = idx[0]
        r = idx[1]
        #indexy bodov kolmice
        x = round( np.cos(fi)*r + x_r )
        y = round( np.sin(fi)*r + y_r )
        data_usecka[x][y] = 255
        #vektor kolmice
        nx0 = np.sin(fi)*np.sign(r)
        ny0 = np.cos(fi)*np.sign(r)
        dlzka_norm = round( (nx0**2 + ny0**2)**(1/2) )

        nx = round(nx0/dlzka_norm,2)
        ny = -round(ny0/dlzka_norm,2)
    ...
    for x in range(w):
        for y in range(h):
            if data_usecka[x][y]==255 or image[x][y]==255 :
                data_vysledok[x][y]=255

    if zobraz_postupnost_bodov:
        Image.fromarray(data_vysledok,'RGB').show()
...
def houghTransfor(pole):
    ...
    data_farba_accum = np.zeros((w,h,3), dtype=np.uint8)
    ...
    # print('Maximalny pocet inkrementácií -',str(maxim))
    ...
    # print('Percento:',str(percento))
    amin = int( percento * maxim )
    # print('Amin:',str(amin))
    amin = int( 1 / 7 * maxim )
...
image = np.asarray( Image.open("imgs/pointCloud4.png"))
w,h = np.shape(image) 

#Vykreslovania
zobraz_postupnost_bodov = False
zobaz_AkumulMaticu = False


#Pociatocne vytvorenie pola bodov, a inicializacia obrazka
data_farba = np.zeros((h,w,3), dtype=np.uint8)
pole_new = []
maxi,maxj = np.shape(image)
pole_bodov = []
povodne_pole = []
for i in range(maxi):
    for j in range(maxj):
...
#zoradenie useciek podla hodnoty inkrementacii
