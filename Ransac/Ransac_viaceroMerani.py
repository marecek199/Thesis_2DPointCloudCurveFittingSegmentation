...
def ransac(pole_bodov,opakovanie,min_pocet_bodov):
    global average,min_vzdial

    omega = 0.2
    z = 0.95

    treshold_ransac = 7*min_vzdial
    treshold_stvorce = 15*min_vzdial
    naslednost_indexov = 10
    if len(pole_bodov) <= naslednost_indexov:
        naslednost_indexov = 1
    ...
    for idx in range(k):
        ...
        # POZOR NA DOSTATOCNE INDEXI    
        i1 = i0+naslednost_indexov
        if i1>=len(pole_bodov):
            i1=i0-naslednost_indexov
        
        # z bodov nasledne vytvara vektor/priamku a k nej zistuje kolko bodov je v 
        # "trehsold" vzdialenosti od priaky zisti kolko bodov patri danej priamke s error<treshold_ransac
        ...
        for bod in pole_bodov:            
            ...
            ndx, ndy = -vektY/normal ,vektX/normal
            vzdialenost_bod_priamka = abs(ndx*rx + ndy*ry)
            if vzdialenost_bod_priamka < treshold_ransac:
                pocet_patriacich+=1
        ...
        if working==False:
            ...
            for bod in pole_bodov:            
                ...
                ndx, ndy = -vektY/normal ,vektX/normal
                vzdialenost_bod_priamka = abs(ndx*rx + ndy*ry)
                if vzdialenost_bod_priamka < treshold_stvorce:
...
def kolinear(pole):
    ...
    tolerancna_medz_bez = -25*min_vzdial
        ...
        for idx in range(len(pole)):
            if working:
                for jdx in range(len(pole)):
                    if idx != jdx: 
                        #viacere totoznych bodov definujucich vektor, velky problem
                        bod = False                    
                        totozne_bod_jdx = True
                        totozne_bod_idx = True
                        for jdx_idx in range(1,len(pole[jdx])):
                            if pole[jdx][jdx_idx] != pole[jdx][jdx_idx-1]:
                                totozne_bod_jdx = False                    
                        for idx_idx in range(1,len(pole[idx])):
                            if pole[idx][idx_idx] != pole[idx][idx_idx-1]:
                                totozne_bod_idx = False
                        if totozne_bod_jdx:
                            a = pole[jdx][0]
                            pole[jdx] = a
                        if totozne_bod_idx:
                            a = pole[idx][0]
                            pole[idx] = a                            
                        if totozne_bod_idx or totozne_bod_jdx:
                            bod = True
...
def delenie_poly_koniec(pole_skupin_bodov,zobrazenie_delenia=False):
    global w,h,average,zobraz_postupnost_bodov
...
minPocetBodov = len(pole_bodov)*0.3
...
if error > 10*min_vzdial:
    while error > 10*min_vzdial:
