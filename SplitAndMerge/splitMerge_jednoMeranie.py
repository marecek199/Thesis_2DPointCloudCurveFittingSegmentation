...
def split(first_idx,last_idx,pole_bodov,farba):
    ...
    '''
    global vykreslenie_kolmice_RG,min_vzdial, povodne_pole

    treshold = 10*min_vzdial
    idx0,idy0 = first_idx
    idx1,idy1 = last_idx
    
    # filter pre pripad pola prvkov rovnakej hodnoty
    vysledok_pole_filt_totoz = []
    if len(pole_bodov) < 150:
        vysledok_pole_filt_totoz = []
    ...
    # pripad nuloveho alebo 1 prvkoveho pola
    if len(pole_bodov)==1 or len(pole_bodov)==0:
        return pole_bodov
    ...
    if vykreslenie_kolmice_RG:
        ...
        for i in range(round(vzdialenost)):
            x += dx
            y += dy
            plt.plot(idx[1],w-idx[0],'.r',markersize=1)
...
def delenie_poly_koniec(pole_skupin_bodov,zobrazenie_delenia=False):
    ...
    treshold_strihania = 30*min_vzdial
...
def kolinear(pole):
    global aver,min_vzdial
    treshold_vzdialenost_priamok = 25*min_vzdial
