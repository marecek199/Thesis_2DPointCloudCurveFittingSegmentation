...
def vykresli_pyplot_farba(vysvietene_pole,povodne_pole,farba,sign):
    ...
    for boddvoj in vysvietene_pole:
        x,y = boddvoj[1], boddvoj[0]
        plt.axis([0,w,0,h])
        plt.plot(x,w-y, marker=sign, color=farba)
...
def split(first_idx,last_idx,pole_bodov,farba):
    ...
    '''
    global vykreslenie_kolmice_RG,povodne_pole0,min_vzdial

    treshold = 10*min_vzdial
    idx0,idy0 = first_idx
    idx1,idy1 = last_idx
    
    # max dlzka priamky
    vzdialenost = round(((idx1-idx0)**2+(idy1-idy0)**2)**(0.5))
    ...
    if vykreslenie_kolmice_RG:
        ...
        for idx in pole_bodov:
            plt.plot(idx[1],w-idx[0],'.b',markersize=2)
...
def merging(pole):
    ...
    treshold_vzdialenost = 10*min_vzdial
    treshold_body = 8*min_vzdial
                        ...
                        if len(pole[idx]) == 1 and len(pole[jdx]) == 1:
                            ...
                            if len(pole[idx]) == 1 or len(pole[jdx]) == 1:
                                ...
                                # pole1 = pole[idx][:]
                                    ...
                                    chyba_spolocne = vzdialenost_bod_maxc / max_dlzka_pole_spolocne
...
def delenie_poly_koniec(pole_skupin_bodov,zobrazenie_delenia=False):
    ...
    treshold_strihania = 10*min_vzdial
...
def kolinear(pole):
    global aver,min_vzdial
    treshold_vzdialenost_priamok = 5*min_vzdial
    tolerancna_medz_so_smernicou = 3*min_vzdial
    tolerancna_medz_bez = -2*min_vzdial
    treshold_body = 6*min_vzdial
...
#Pociatocne vytvorenie pola bodov
